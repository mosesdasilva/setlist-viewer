...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and awaiting final review. Section Bands with a uniform Grid won; the Bar Bubble treatment and its toggle were removed. Section Names run vertically in a narrow left rail with the Section Code at the bottom. All Grid dividers are one pixel. A sticky-header Song Map shows every Section occurrence as a colored circular link, scrolls horizontally on phones, uses a non-clipping inset selected ring, and dynamically offsets jumps below the header. Wide screens support adjustable one/two-column layout; narrow screens force one column. One-row Tags align with two-Chart-Row Sections. A Melody toggle hides/shows borderless red Melody Fragment capsules without hiding Row Notes. A palette toggle switches Strong and Pastel Section colors independently from dark-blue and off-white page themes. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
