...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and awaiting final human review. Section Bands won; an icon preserves both subtle Grid cells and rounded Bar Bubble cells for comparison. Bar Bubbles now use transparent fill and dark outlines matching the compact Section Code chip. Wide screens support adjustable one/two-column layout; narrow screens force one column. One-row Tags align with two-Chart-Row Sections. Melody Fragments use borderless red capsules with white text. Octave Cues appear as plain Row Notes without an added melody label. Themes retain identical saturated bands and white lettering over dark-default or pure-white backgrounds. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
