...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and near final human review. Section Bands won. Section Names run vertically in a narrow left rail, removing the former top header row; the Section Code sits at the rail's bottom. Grid cells are the default/leading candidate with uniform one-pixel dividers, including the Bar/Row Note boundary; the icon preserves softly filled rounded Bar Bubble cells for comparison. A sticky-header Song Map shows every Section occurrence left-to-right as a colored circular link and scrolls horizontally on phones. Wide screens support adjustable one/two-column layout; narrow screens force one column. One-row Tags align with two-Chart-Row Sections. Melody Fragments use borderless red capsules with white text; Octave Cues appear as plain Row Notes. Themes retain identical saturated bands and white lettering over dark-default or pure-white backgrounds. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
