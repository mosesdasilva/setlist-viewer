...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and awaiting final review. Section Bands with a uniform Grid won; the Bar Bubble treatment and its toggle were removed. Section Names run vertically in a narrow left rail with the Section Code at the bottom. All Grid dividers are one pixel. A sticky-header Song Map shows every Section occurrence as a colored circular link, scrolls horizontally on phones, uses a non-clipping inset selected ring, and dynamically offsets jumps below the header. Wide screens support adjustable one/two-column layout; narrow screens force one column. One-row Tags align with two-Chart-Row Sections. Chord Symbols stay centered independently while single-line Melody Fragment capsules sit at the Bar bottom; the Melody toggle changes only their visibility. Capsules are borderless and shadowless: saturated red with white text in Strong mode, softer rose with chart-matching dark text in Pastel mode. The palette toggle works independently from dark-blue and off-white page themes. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
...
- Review the refined Section Bands prototype once more before resolving its HITL ticket.
