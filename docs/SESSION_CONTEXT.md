...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and awaiting final human review. Section Bands won: the refinement uses adjustable one/two-column wide-screen layout, forced single-column narrow layout, equal visual height for one- and two-Chart-Row Sections, gridless full-color bands with only a Chords/Row Note divider, identical saturated band colors and white lettering across themes, vivid red Bar-aligned Melody Fragments, a dark-default/pure-white-background light theme, and icon-only layout/theme controls. The Row Note column remains reserved for Performance Directions. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
