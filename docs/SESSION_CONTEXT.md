...
- [Prototype the expanded responsive chart view](https://github.com/mosesdasilva/setlist-viewer/issues/6) is claimed and awaiting final human review. Section Bands won. Section Names now run vertically in a narrow left rail, removing the former top header row; the Section Code sits at the rail's bottom. Grid cells are the default/leading candidate, while the icon preserves softly filled rounded Bar Bubble cells for comparison. Wide screens support adjustable one/two-column layout; narrow screens force one column. One-row Tags align with two-Chart-Row Sections. Melody Fragments use borderless red capsules with white text; Octave Cues appear as plain Row Notes. Themes retain identical saturated bands and white lettering over dark-default or pure-white backgrounds. It lives in `src/responsive-chart-prototype.html` on `prototype/responsive-chart-view`.
